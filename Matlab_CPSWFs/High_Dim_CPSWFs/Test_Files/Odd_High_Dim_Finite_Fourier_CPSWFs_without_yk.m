...
function S=Odd_High_Dim_Finite_Fourier_CPSWFs_without_yk(r,k,c,Truncation_Number,n,dim)
S=0;
N=Odd_High_Dim_Coefficient_CPSWFs(k,c,Truncation_Number,n,dim);
for j=1:length(N)
    if N(j)>1/1000000
    S=S+N(j)*((-1)^(k)*2^(2*(j-1)+1)*factorial(2*(j-1)+1).*(1i)^(k+2*(j-1)+1)*1/sqrt(2*pi)*((-1)^(j))./(c.*r.^(dim/2)).*besselj(k+2*j-1+dim/2,2*pi.*r)).*(sqrt(2*k+4*(j-1)+2+dim))/(2^(2*(j-1)+1)*factorial(2*(j-1)+1));
    end
end