...
function M=Even_High_Dim_CPSWFs_Matrix(k,c,matrix_size,dim)
    
M=zeros(matrix_size,matrix_size);

M(1,1)=4*pi^2*c^2*(k+dim/2)/(k+dim/2+1);

for i=1:matrix_size-1
    
    M(i,i+1)=-(4*(pi)^2)*(c^2)*(i*(k+i-1+dim/2))/((k+2*i-2+dim/2+1)*sqrt((k+2*i-2+dim/2+2)*(k+2*i-2+dim/2)));
    
    M(i+1,i+1)=4*i*(k+i+dim/2)+4*pi^2*c^2/(k+2*i+dim/2)*((k+i+dim/2)^2/(k+2*i+dim/2+1)+i^2/(k+2*i+dim/2-1));
    
    M(i+1,i)=-(4*(pi)^2)*(c^2)*(i*(k+i+dim/2-1))/((k+2*i+dim/2-1)*sqrt((k+2*i+dim/2)*(k+2*i+dim/2-2)));
