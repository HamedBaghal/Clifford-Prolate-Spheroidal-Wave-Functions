...
C=((((2.^(2*N)))*gamma((2*N)+1))/(gamma(N+1)))*((gamma(k+dim/2+N)/(gamma(k+dim/2)))*(((r.^k)/(sqrt(2*pi)))));
for i=1:N
C=C+((((2^(2*N)))*gamma((2*N)+1))/(gamma(N+1)))*((nchoosek(N,i)*((gamma(i+k+dim/2+N))/(gamma(i+k+dim/2)))*((-1)^i)*(1/(sqrt(2*(pi))))*(r.^(2*i+k))));
end
end
