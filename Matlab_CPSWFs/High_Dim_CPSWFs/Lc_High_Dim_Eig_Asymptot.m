...
T=zeros(1,10);
for n=1:5
    M=Even_High_Dim_CPSWFs_Matrix(k,0.1,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T1=abs(N(1)-M(1,1));
    
    M=Even_High_Dim_CPSWFs_Matrix(k,0.2,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T2=abs(N(1)-M(1,1));    
    ...
    M=Odd_High_Dim_CPSWFs_Matrix(k,0.1,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T1=abs(N(1)-M(1,1));
    
    M=Odd_High_Dim_CPSWFs_Matrix(k,0.2,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T2=abs(N(1)-M(1,1));    
...
for n=1:5
    ...
    M=Even_High_Dim_CPSWFs_Matrix(k,0.3,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T2=abs(N(1)-M(1,1));    
    ...
    M=Odd_High_Dim_CPSWFs_Matrix(k,0.3,m,dim);
    [V,D]=eig(M);
    [S,I]=sort(diag(D));
    N=S(:,1);
    T2=abs(N(1)-M(1,1));    
