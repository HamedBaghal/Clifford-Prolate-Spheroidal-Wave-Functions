...
for k=0:40
W=abs(Even_High_Dim_Eigenvalue_CPSWFs(k/10,1,100,1,dim));
U=abs((Odd_High_Dim_Eigenvalue_CPSWFs(k/10,1,100,1,dim)));
plot(k/10,W,'or','LineWidth',3)
hold on
plot(k/10,U,'*k','LineWidth',3)
end
M=abs(Even_High_Dim_Eigenvalue_CPSWFs(k/10,1,100,1,dim));
N=abs(Odd_High_Dim_Eigenvalue_CPSWFs(k/10,1,100,1,dim));
