...
function D=Even_High_Dim_Clp_without_Yk(r,N,k,dim)
...
C=((((2.^(2*N)))*gamma((2*N)+1))/(gamma(N+1)))*((gamma(k+dim/2+N)/(gamma(k+dim/2)))*(((r.^k)/(sqrt(4*pi)))));
for i=1:N
C=C+((((2^(2*N)))*gamma((2*N)+1))/(gamma(N+1)))*((nchoosek(N,i)*((gamma(i+k+dim/2+N))/(gamma(i+k+dim/2)))*((-1)^i)*(1/(sqrt(4*pi)))*(r.^(2*i+k))));
end
end
