...
C=-((((2.^((2*N)+1))).*gamma((2*N)+2))./(gamma(N+1))).*(gamma(k+N+dim/2+1)./(gamma(k+dim/2+1)).*(((r.^(k+1))./(sqrt(4*pi)))));
for i=1:N
C=C-((((2.^((2*N)+1))).*gamma((2*N)+2))./(gamma(N+1))).*((nchoosek(N,i).*((gamma(i+k+N+dim/2+1))./(gamma(i+k+dim/2+1))).*((-1).^i).*(1/(sqrt(4*pi))).*(r.^(2*i+k+1))));
end
end
