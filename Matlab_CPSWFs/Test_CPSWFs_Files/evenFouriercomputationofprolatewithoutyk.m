...
function S=evenFouriercomputationofprolatewithoutyk(r,k,c,m,n)
S=0;
N=evencoefficientcliffordprolate(k,c,m,n);
for j=1:length(N)
    if N(j)>1/10000
    S=S+N(j)*((-1)^(k)*2^(2*(j-1))*factorial(2*(j-1)).*(1i)^(k+2*(j-1))*1/sqrt(2*pi)*((-1)^(j-1))./r.*besselj(k+2*j-1,2*pi.*r)).*(sqrt(2*k+4*(j-1)+2))/(2^(2*(j-1))*factorial(2*(j-1)));
    end
end